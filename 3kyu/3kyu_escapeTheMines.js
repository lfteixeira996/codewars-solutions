...
// map is a 2-dimensional array of boolean values, representing squares. false
// for walls, true for open squares (where the miner can walk). It will never be
// larger than 5 x 5. It is laid out as an array of columns. All columns will
// always be the same size, though not necessarily the same size as rows (in
// other words, maps can be rectangular). The map will never contain any loop,
// so there will always be only one possible path. The map may contain dead-ends
// though.

// miner is the position of the miner at the start, as an object made of two
// zero-based integer properties, x and y. For example {x:0, y:0} would be the
// top-left corner.
...
class Johnny {
  ...
  walk(x, y) {
    ...
    let currentMovement = moveLeft
      ? "left"
      : moveUp ? "up" : moveRight ? "right" : moveDown ? "down" : false;
